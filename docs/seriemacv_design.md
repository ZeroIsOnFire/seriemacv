...
The implemented server uses the official Python SDK v2 over local `stdio`. One
process is bound to one validated project root. Read tools, `seriemacv://` resources,
and prompts never mutate the project.

### Read and proposal tools
...
Every `prepare_*` write returns a validated diff and a short-lived, single-use token.
Only `confirm_change` applies it, after checking the affected-file hashes. Canonical
career editing uses typed operations rather than raw YAML or JSON Patch and includes
deletion cascades in the same preview.

`prepare_browser_application` is the high-risk boundary: preparation itself does not
launch Playwright, while token confirmation may open and fill the isolated local
session. There is deliberately no browser-submission tool. CAPTCHA bypass and claims
of external success are outside the product contract.
