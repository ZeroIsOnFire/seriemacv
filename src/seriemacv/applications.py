...
class ApplicationDocument(StrictModel):
    ...
    def identifiers_unique(self) -> "ApplicationDocument":
        for name, records in (("answers", self.answers), ("questions", self.questions)):
            values = [
                record.field_id if name == "answers" else record.id
...
def load_application(project_path: Path, application_id: str) -> ApplicationDocument:
    ...
    if result.id != application_id:
        raise ValueError(
            f"application declares '{result.id}', expected '{application_id}'"
        )
...
def update_status(
    project_path: Path, application_id: str, status: ApplicationStatus
) -> ApplicationDocument:
...
def add_questions(
    project_path: Path, application_id: str, questions: list[ApplicationQuestion]
) -> ApplicationDocument:
    document = load_application(project_path, application_id)
    ...
    updated = document.model_copy(
        update={
            "questions": [*document.questions, *questions],
            "status": status,
            "audit": [
                *document.audit,
                _audit("questions_detected", str(len(questions))),
            ],
        }
    )
    _write(application_path(project_path, application_id), updated)
...
def replace_questions(
    ...
    updated = document.model_copy(
        update={
            ...
            "audit": [
                *document.audit,
                _audit("questions_refreshed", str(len(pending))),
            ],
        }
    )
    _write(application_path(project_path, application_id), updated)
...
def propose_answer(
    ...
    if unknown := set(evidence_ids) - verified:
        raise ValueError(
            f"proposal references unverified or unknown evidence: {', '.join(sorted(unknown))}"
        )
    questions = []
    found = False
...
def apply_answer(
    ...
    *,
    ...
    record = ApplicationAnswer(
        ...
        answer=resolved,
        saved_answer_id=save_answer_id,
        sensitive=question.sensitive,
        confirmed_for_application=True,
    )
    answers = [
        item for item in document.answers if item.field_id != question.field_id
    ] + [record]
    questions = [item for item in document.questions if item.id != question_id]
    status: ApplicationStatus = (
        "ready_for_review" if not questions else "needs_user_input"
    )
    updated = document.model_copy(
        update={
            "answers": answers,
            "questions": questions,
            "status": status,
            "audit": [*document.audit, _audit("answer_confirmed", question.id)],
        }
    )
    _write(application_path(project_path, application_id), updated)
    return updated
...
def set_cover_letter_path(
    project_path: Path, application_id: str, relative_path: str
) -> ApplicationDocument:
    document = load_application(project_path, application_id)
    updated = document.model_copy(
        update={
            "cover_letter_path": relative_path,
            "audit": [*document.audit, _audit("cover_letter_accepted")],
        }
    )
...
def dump_application(
    value: ApplicationDocument | list[ApplicationDocument] | list[ApplicationQuestion],
) -> str:
    stream = StringIO()
    dumped = (
        [item.model_dump(mode="python") for item in value]
...
def _validate_links(project_path: Path, document: ApplicationDocument) -> None:
    ...
    for relative in [
        ...
        *([document.cover_letter_path] if document.cover_letter_path else []),
    ]:
        resolved = (root / relative).resolve()
        if not resolved.is_relative_to(root) or not resolved.is_file():
            raise ValueError(f"attachment path is invalid: {relative}")
    answers = {
        item.id: item for item in load_career(project_path / "career.yml").answers
    }
