...
from seriemacv.privacy import REDACTED, redact_sensitive_text
...
class PrivacyTests(unittest.TestCase):
    def test_redacts_credentials_personal_data_and_sensitive_field_values(self) -> None:
        ...
        for secret in ("secret-token", "abc123", "hunter2", "session-value", "person@example.invalid", "+55 11 95555-0100", "5000"):
    ...
    def test_cli_error_redacts_reflected_personal_data(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with patch("seriemacv.cli.set_profile", side_effect=ValueError("email: person@example.invalid token=abc123")):
                from contextlib import redirect_stderr
                from io import StringIO

                with redirect_stderr(StringIO()) as output:
                    result = main(["career", "set-profile", str(project_path), "--name", "Example"])
    ...
    def test_studio_error_redacts_response_body(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            project_path = Path(temporary_directory) / "career"
            create_project(project_path, project_name="Career")
            with patch("seriemacv.studio.load_jobs", side_effect=ValueError("phone: +55 11 95555-0100")):
    ...
    def test_mcp_error_redacts_message(self) -> None:
        with patch("seriemacv.mcp._call", side_effect=ValueError("email: person@example.invalid")):
            response = _handle({
                "jsonrpc": "2.0", "id": 7, "method": "tools/call",
                "params": {"name": "list_jobs", "arguments": {"project_path": "ignored"}},
            })
